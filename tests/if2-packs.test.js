...
// --- 1) subset enumeration (31 non-empty subsets of gens 1-5) ------------------
{
  const out = execFileSync('python3', ['build-if2-packs.py', '--list-subsets'],
    { cwd: root, encoding: 'utf8' });
  const keys = JSON.parse(out);
  ok(keys.length === 31, `1: 31 subsets (got ${keys.length})`);
...
// --- 2) gen→PIF-id pool computation --------------------------------------------
{
  ...
  if (hasIf2) {
    ...
    const union = JSON.parse(execFileSync('python3', ['-c',
      ...
      { cwd: root, encoding: 'utf8', env: { ...process.env, CC_SPECIES_GENS: '1,2,3,4,5', CC_SPLITNAMES_RB: splitnames } }));
    ok(union[0] === 507 && union[1] === 570, `2: union pool 507 species, max 570 (got ${union[0]}/${union[1]})`);
    ok(union[2].length === 33, `2: union legendaries = 33 (got ${union[2].length})`);
    void env;
  } else {
...
function section5() {
  ...
  // checkForUpdate: same contentVersion/sha but different gens → 'available'.
  ls['cc.contentPack.creature-if2.v1'] = JSON.stringify({
    packId: 'creature-if2', contentVersion: 'v1', sha256: 'abc', gens: '1',
    installedAt: 1,
  });
  ...
  return (async () => {
    ...
    ok(r1.state === 'available', '5: gens mismatch → update available (got ' + r1.state + ')');
    P.setSelectedGens('creature-if2', [1]);
    const r2 = await PI.checkForUpdate('creature-if2');
    ok(r2.state === 'up-to-date', '5: matching gens → up-to-date (got ' + r2.state + ')');
...
function section6() {
  ...
  ok(packsSrc.includes('genRange') && packsSrc.includes('cc.packGens.'),
    '6: packs.js genRange + cc.packGens storage');
}
