#!/usr/bin/env bash
# Upload the built content pack (packs/creature-fusion/) to the Hugging
# Face creature-pack dataset.
#
...
#
# Usage:
#   scripts/upload-content-pack.sh [repo-id]        # default TessaCoil/creature-pack
...
if [ ! -f packs/creature-fusion/pack.bin ]; then
  echo "ERROR: packs/creature-fusion/pack.bin not found — run build-content-pack.py first" >&2
  exit 1
fi
...
exec hf upload-large-folder "$REPO_ID" packs/creature-fusion/ \
  --repo-type=dataset \
