...
**Parallelism:** added `--jobs/-j` (default 1 serial; 0 = all CPUs) to `--all`. ~9,801 pairs are independent → `multiprocessing.Pool` with a `_worker_init` that loads the bundle once per worker. Serial ≈ 0.8 pairs/s (~3h); 60 workers ≈ minutes. `--render-samples` is ignored when `--jobs>1`.

**To regenerate after any sprite/cells change:** `python build-shiny-palettes.py --all --jobs 0 && python shiny-palettes-to-bin.py`. Related: [[reference_pokedex_architecture]].
